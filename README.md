...
- **2026-07-02** 🚀 **v1.7.2 — i18n 品牌双语统一 · 首页视觉全面升级（蓝图视觉规范落地）**：
  ...
  - **首页视觉全面升级（蓝图视觉规范落地）**：依据 `docs/tide_trading_grand_roadmap.md` 中 §2 界面视觉设计规范，对首页进行全量视觉重构：
    - **英雄区**：超大渐变标题（橙→琥珀），三色霓虹光晕背景装饰（橙/青/粉），双 CTA 按钮（主级 + 次级量化大屏入口），新增**实时行情 Ticker** 展示 A 股核心标的涨跌行情（宁德时代、茅台、比亚迪等），绿涨红跌符合 A 股配色铁律。
    - **特性卡片**：全部重构为玻璃态容器（`backdrop-blur-sm`），四张卡片各赋独立霓虹主题色（青色/橙色/品红/琥珀），Hover 时触发角落发光特效。
    - **LLM 编排步骤**：四步骤采用霓虹彩色数字圆环区分，整体置于毛玻璃背景卡片中。
    - **A/H 数据链卡片**：A股卡片采用玫瑰红霓虹主题（符合 A 股涨跌铁律），港股风控链卡片采用青色霓虹主题（赛博朋克科幻风），两卡片均带 Hover 发光阴影。
    - **引导向导 & 路线图**：步骤大号霓虹数字水印 + 发光时间线节点（`animate-pulse`），双栏已完成/规划蓝图对照布局。
  - **大屏 Header 品牌双语**：数据大屏顶部 Header 主标题中文模式显示「潮汐投研 · 量化工作站」，副标题「实时多智能体量化看板」；英文模式自动切回「TIDETRADING QUANT STATION」。
...
- **2026-07-02** 🚀 **v1.7.1 — 共享数据底座、大屏全卡片玻璃态重构与数据清洗自愈版本发布**：
  ...
  - **本地 SQLite 数据库占位符一键清洗**：编写并集成了 SQLite 数据库清洗脚本。直接对容器及主机端的 SQLite 文件中的 `Watchlist` 和 `stock_meta` 两个关键表进行大洗牌，将占位名称全部替换为真实中文股票简称，实现本地数据层自愈。
  - **腾讯实时行情分批（Chunk）抓取**：优化 API 行情拉取，将大容量自选股列表自动切分为 50 只个股为一组的分批请求，规避超长 URL 超时问题。
  - **共享/私有双层数据库路由解耦**：股票基础元数据、全市场历史 K 线数据统归于共享库 `stocks_market.db`，用户自选股与回测轨迹严格物理隔离于租户库 `stocks_<tenant_id>.db`。
  - **通达信二进制行情高速冷启动导入**：编写本地 `.day` 二进制行情文件导入器，以 SQLite WAL 模式实现平均每秒 2.8 万条的高速增量回填，累计注入 **510 万+ 条** 历史日线数据。
  - **写锁防死锁保护机制**：冷启动写入期间并发读请求遇到 `database is locked` 时自动跳过，保证大屏和回测服务全天候可用。
  - **首屏 i18n 中文默认拦截**：首次访问自动预设 `zh-CN`，确保 100% 呈现中文看板大屏。
  - ⚠️ **独立正名**：项目正式从上游 Fork 独立，版本号切换至 `vx.y.z` 标准语义化版本体系。
