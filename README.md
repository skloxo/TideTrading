...
- **2026-06-30** 🚀 **v0.1.10.cnx.1.4 — 首页品牌定位优化与迭代历史规范化发布**：
  - **产品品牌定位宣导打磨**：根据 CNX 分支特色（深耕 A 股与港股、融入社交舆情监控及资金大单、支持多租户隔离沙箱与多通道协同推送等）对首页 Hero 核心文案进行了精细化提炼。
  - **操作入口精炼与冗余按钮裁剪**：精简了首页的冗余操作按钮，去除了顶部 Hero Section 的“实盘风控监视”按钮，以及“新手引导向导”底部的“立即开启量化对话”重复链接，使租户体验更纯粹聚焦。
  - **迭代记录规范化与补齐**：在里程碑时间轴上补齐了 `v0.1.10.cnx.1.0`（量化工作站首发版）节点，重命名与规范化了迭代历史的命名格式。
- **2026-06-30** 🚀 **v0.1.10.cnx.1.3 — 服务看板深度集成与交互优化版本发布**：
  - **管理设置与看板功能合并**：将原位于设置页底部的“租户密钥管理 (Tenant API Keys)”和“系统版本管理”功能合并迁移至“服务看板 (`/monitor`)”页面，左列展示租户密钥与工作区管理表格，右列展示行情网关状态与系统版本管理，实现一体化系统监控与配置运维。
  - **用户密钥隐私安全性强化**：移除了租户密钥列表中明文与掩码密钥的直接渲染，改为精致小巧的“复制密钥”安全气泡按钮，防止界面凭证意外被旁观者窃视；同时在自助注册模态框中移除了对设置页的冗余密钥拉取副作用。
  - **前端交互反馈与体验升级**：在行情网关的“刷新状态”按钮与系统版本的“检查更新”按钮被手动触发时，全面补齐了成功与失败的 Toast 动画提示与加载转轮动画，杜绝按钮无响应的交互痛点。
