...
<h1 align="center">Vibe-Trading-CNX：港深定制版量化交易多智能体工作站</h1>

<p align="center">
  <b>深度整合 A股/港股，多通道智能体团队协同（Swarm）与安全沙箱投研工作站</b>
</p>
...
> [!NOTE]
> 本项目是 [Vibe-Trading](https://github.com/HKUDS/Vibe-Trading) 的高级定制分支版本（超集）。在深度整合 **A股和港股** 市场专属特性（如微信 iLink / 飞书多通道推送、中文 Web UI、A股题材复盘方法论等）的同时，**彻底恢复并加固了全部全球境外市场（美股、加密货币、外汇、期货）与 10 大券商连接器（OKX, Binance, Alpaca, IBKR 等）的代码与回测测试**，并实现了多租户安全沙箱隔离机制与基于 Host 请求头的局域网自动安全提权，跑通 100% 单元与安全测试。
...
我们将按照量化研究员与交易员在 A股与港股市场的实际痛点，分阶段交付与增强系统：

| 阶段 | 核心模块 / 特性 | 状态与进展 |
|------|------|------|
| **1. Localized Infrastructure** | 中文本地化 UI、Tushare 与 Akshare 免密/免 Token 通道整合、东方财富/新浪数据接口集成 | **已完成** |
| **2. Multi-channel Briefings** | 飞书（Lark）与微信（iLink）多通道即插即用机器人集成，支持 `/myid` 反射、异常心跳通知，以及主动式定时研究 Briefs 推送 | **已完成 (v0.1.10.cnx.1)** |
| **3. Security & Sandboxing** | 多租户安全隔离（限制于租户专属物理目录、限制 BashTool 执行），并引入 `API_ALLOWED_HOSTS` 局域网与公网 CORS 提权放行机制 | **已完成 (v0.1.10.cnx.1)** |
| **4. Trading Connectors & Execution** | 提供模拟交易、富途证券 (Futu)、盈透证券 (IBKR) 等 10 大券商的可视化配置面板、状态心跳实时监测与自动交易路由 | **开发中 / 下一步** |
| **5. Options & Derivatives Lab** | 针对期权交易的隐含波动率曲面（Vol Surface）、希腊字母（Greeks）实时面板、组合损益与情景模拟器 | **规划中** |
| **6. Swarm Strategy Factory** | 多智能体量化策略梦工厂：自动生成多智能体协同运行的量化交易班子、定制化组合优化器与每日 A股题材复盘 Brief Autopilot | **规划中** |
