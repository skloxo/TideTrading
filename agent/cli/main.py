"""Interactive CLI front door for TideTrading.
...
The console script entry in ``pyproject.toml`` (``tide = "cli:main"``)
hits :func:`main`.
"""
...
def _probe_session_count() -> int:
    ...
    db_path = Path.home() / ".tide" / "sessions.db"
...
def _show_banner() -> None:
    ...
    console.print(
        ...
        "[/dim][bold]tide connector --help[/bold][dim] · "
...
def _new_session(prompt_preview: str) -> Optional[str]:
    ...
    (``~/.tide/sessions.db``) so cross-session search via
...
def _run_one_turn(user_input: str, ctx: InteractiveContext) -> None:
    ...
    try:
        ...
        dashboard.close()
        # BrokenPipe: caller did ``tide chat | head`` and the
...
def _trip_halt_from_repl(console: Any, *, reason: str) -> None:
    ...
    console.print(
        ...
        "disabled until you run [bold]/resume[/bold] or [bold]tide connector resume[/bold]."
    )
...
def _clear_halt_from_repl(console: Any) -> None:
    ...
    path so the model never performs it. Mirrors ``tide connector resume``
...
def _run_connector_command_from_repl(console: Any, args: list[str]) -> None:
    ...
    ``/connector`` is a thin in-REPL bridge to the ``tide connector``
...
def _interactive_loop(max_iter: int, resume_session_id: Optional[str] = None) -> int:
    ...
    if resume_session_id:
        # Resume a specific session by ID (``tide resume <session-id>``).
        try:
            store = _session_store()
    ...
    if ctx.session_id:
        console.print(
            f"[dim]To resume this session:[/dim] [bold]tide resume {ctx.session_id}[/bold]"
        )
    return 0
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    * Non-interactive entry (``serve``, ``run -p ...``, ``mcp``,
    ...
    # Handle ``tide resume <session-id>`` — enter the interactive
...
def _build_typer_app():  # type: ignore[no-untyped-def]
    ...
    app = typer.Typer(
        ...
        help="TideTrading — natural-language finance research agent.",
