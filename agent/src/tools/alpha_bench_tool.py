...
Cache integrity note: the universe panel cache lives in ``~/.tide-trading/cache/``
...
def _load_universe_panel(
    ...
    Args:
        ...
        use_cache: When True (default) reuse a pickle in
            ``~/.tide-trading/cache/`` if the same universe+period was fetched
...
class AlphaBenchTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "output_dir": {
                "type": "string",
                "description": "Where to write the HTML report; default ~/.tide-trading/reports/.",
            },
        },
