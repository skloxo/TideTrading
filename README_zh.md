...
- **2026-07-10** 🚀 **v1.7.6 — 恢复三位版本号规范、全量消息通道 SDK 打包与内存泄露/残留任务强力清理**：
  - **版本号规范调整**：遵照用户要求恢复标准三位制 `vX.Y.Z` 版本号格式，升级至 `v1.7.6`，明确以 Z 位做日常微小更新与自动化增量。
  - **全量消息通道 SDK 打包**：在 `Dockerfile` 中启用 `[channels]` 依赖项组安装，将钉钉、QQ 等全部第三方消息适配器 SDK 依赖预装至生产与开发环境镜像中，实现前端“不可用通道数”归零。同时修正了 `slackify-markdown` 的 PyPI 版本限制至 `>=0.2.0` 以消除安装错误。
  - **删除/停用租户残留任务清理**：在租户管理 CRUD 接口中实装了内存重载，删除或停用租户时将强制刷新内存中的通道状态并销毁失效的 WeChat iLink 轮询子协程，彻底杜绝无密钥租户在后台持续轮询产生的无限报错。
  - **雪球盯哨生命周期整合**：将大V盯哨主协程 `XueqiuWatcher` 优雅注入 FastAPI 的 lifespan 启动/停止生命周期中，并且使其在每次轮询时动态加载最新的有效租户配置，兼顾历史任务性能与实时性。
  - **通达信配置持久化与静音启动**：修复了 `.mootdx` 配置文件在容器销毁重建时易丢失的问题，通过在 Dockerfile 中将其符号链接映射至持久化数据卷下解决。同时，在启动预检测速 `config.setup()` 时对无关报错进行了静音输出过滤，提供干净整洁的启动控制台体验。
...
- **2026-07-07** 🚀 **v1.7.5.5 — 服务看板重构与系统视觉圆角及信息密度全局统一规范**：
  ...
  - **全局视觉规范统一**：基于 Robinhood 卡片对系统视觉规范进行收窄，将全部圆角从 16px 粗钝度统一收拢为 6px（`rounded-md`）；全局缩小 Padding 与 Gap，整体提升大屏及数据网格的信息密度。
  - **磨砂玻璃态重构**：优化控制台背景及呼吸灯高亮阴影，使亮色/暗色主题无缝融合。
...
- **2026-07-07** 🚀 **v1.7.5.4 — 多租户沙箱隔离、全局持仓收益共享缓存与 Token 契约自愈保护**：
  ...
  - **Token 契约状态联动**：重构 Token 共享池准入规范，当租户 Token 过期失效（或服务关闭、订阅为空）时，系统自动判定为“等同于停用状态”，将其 Token 彻底移出轮询队列并阻断该租户的事件推送，直到其在前端更新凭证自愈。
...
- **2026-07-06** 🚀 **v1.7.4 — 项目设置独立页面 · 租户敏感凭证物理隔离（隐私防漏泄版本）**：
  - **项目设置独立化**：将项目全局 LLM 及数据源默认设置从普通设置中彻底剥离，提取为专属的 [ProjectSettings.tsx](file://wsl.localhost/Ubuntu-24.04/home/skloxo/aho/openclaw/project/TideTrading/frontend/src/pages/ProjectSettings.tsx) 单页（路由 `/project-settings`），支持管理员就地提权访问。
...
- **2026-07-03** 🚀 **v1.7.3 — 管理员就地提权卡片 · 独立租户管理页面上线（操作流失防范版本）**：
  ...
  - **独立租户管理页面**：将原服务看板的租户密钥管理解耦，升级为独立的 [TenantManagement.tsx](file://wsl.localhost/Ubuntu-24.04/home/skloxo/aho/openclaw/project/TideTrading/frontend/src/pages/TenantManagement.tsx)（路由 `/tenants`），并于侧边栏“项目运维”中集成“租户管理”二级导航。
